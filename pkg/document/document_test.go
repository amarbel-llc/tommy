...
// #109: UndecodedKeys must descend inline-table values so an inner entry the
// decoder couldn't consume is reported (instead of silently lost), mirroring
// how an unconsumed [a.b] sub-table surfaces.
func TestUndecodedKeysDescendsInlineTables(t *testing.T) {
	cases := []struct {
		name     string
		input    string
		consumed []string
		...
		{
			// Outer key consumed, one inner entry dropped (type mismatch): the inner
			// must be reported at its full dotted path.
			name:     "dropped inner entry under consumed outer",
			input:    "actions = { build = { command = \"make\" }, deploy = \"oops\" }\n",
			consumed: []string{"actions", "actions.build", "actions.build.command"},
			want:     []string{"actions.deploy"},
		},
		{
			// Fully decoded: nothing undecoded.
			name:     "fully consumed inline table",
			input:    "env = { FOO = \"a\", BAR = \"b\" }\n",
			consumed: []string{"env", "env.FOO", "env.BAR"},
			want:     nil,
		},
		{
			// Entirely unknown inline table: report the single key, do not enumerate.
			name:     "unconsumed inline table reported as one key",
			input:    "extra = { a = 1, b = 2 }\n",
			consumed: nil,
			want:     []string{"extra"},
		},
		{
			// Deep nesting: an unconsumed leaf two levels into the inline table.
			name:     "deep inline nesting",
			input:    "s = { name = \"a\", inner = { x = 1, y = 2 } }\n",
			consumed: []string{"s", "s.name", "s.inner", "s.inner.x"},
			want:     []string{"s.inner.y"},
		},
		{
			// Inline table nested under a consumed table header (the #115 shape).
			name:     "inline under consumed header",
			input:    "[exec]\nsession = { name = \"a\", note = \"b\" }\n",
			consumed: []string{"exec", "exec.session", "exec.session.name"},
			want:     []string{"exec.session.note"},
		},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, k := range tc.consumed {
				consumed[k] = true
			}
			got := UndecodedKeys(doc.Root(), consumed)
			if len(got) != len(tc.want) {
				t.Fatalf("UndecodedKeys = %v, want %v", got, tc.want)
			}
			for i := range got {
				if got[i] != tc.want[i] {
					t.Fatalf("UndecodedKeys = %v, want %v", got, tc.want)
