...
// RenderFile renders the generated *_tommy.go file for the given structs using
// jennifer (the sole codegen renderer).
func RenderFile(w io.Writer, pkg string, structs []StructInfo) error {
	// #84 migration scaffold: TOMMY_COMP_RENDERER routes generation through the
	// compositional renderer (comp_*.go) so the full integration suite can gate
	// it before the Step-D flip. Removed once the compositional path is the only
	// one.
	if os.Getenv("TOMMY_COMP_RENDERER") != "" {
		return compRenderFile(w, pkg, structs)
	}
	f := jen.NewFile(pkg)
	f.NoFormat = true
...
func jenSetSlicePrimitive(ctx encCtx, o SetSlicePrimitive, cv *jen.Statement) []jen.Code {
	bk := o.TKey.BareKey()
	// Without omitempty the field always serializes — an empty slice must emit
	// the explicit "key = []" form, not be dropped. See #82.
	if !o.OmitEmpty {
		return []jen.Code{jen.BlockFunc(func(g *jen.Group) {
