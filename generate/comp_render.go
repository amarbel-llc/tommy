...
// Compositional renderer (#84). compRenderFile walks the cd*/ce* node trees
// (comp_build.go) and emits the generated *_tommy.go. It reuses the decode/encode
// contexts (jenCtx/encCtx) and the shared jennifer helpers (tableMatch,
// posHeader, jenType, jenZeroLit, delegateParts, cst/doc package consts) from the
// enumerated renderer; Step D preserves those and deletes only the op-walk
// functions. The four gaps the spike dropped — consumed tracking, handle
// tracking, positional nesting (#10), and the flat-key fallback (#55) — are
// closed here by transplanting the enumerated renderer's correct bodies onto the
// compositional walk.
...
func compRenderFile(w io.Writer, pkg string, structs []StructInfo) error {
	f := jen.NewFile(pkg)
	f.NoFormat = true
